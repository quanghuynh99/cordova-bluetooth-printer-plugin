...
@property (nonatomic, strong) CBCharacteristic *chatacter;         
@property (nonatomic, strong) NSMutableArray *servicesArray;       
@property (nonatomic, strong) NSMutableArray *printerModelArray; 
@end
...
- (void)setPrinterPageWidth:(CDVInvokedUrlCommand *)command{
    if (command.arguments.count > 0 && command.arguments[0] != [NSNull null]) {
        ...
        if (width > 0) {
            ...
            [self callBackSuccess:YES callBackId:command.callbackId message:[NSString stringWithFormat:@"Set paper width successfully:%ld",width]];
            return;
        }
    }
...
- (void)setFirstRankMaxLength:(CDVInvokedUrlCommand *)command{
    if (command.arguments.count > 1) {
        ...
        [self callBackSuccess:YES callBackId:command.callbackId message:[NSString stringWithFormat:@"Setup successful"]];
        return;
    }
    [self callBackSuccess:NO callBackId:command.callbackId message:@"The parameter is a numeric array, for example [16,20]"];
...
- (void)autoConnectPeripheral{
    ...
    if (peripheralName) {
        ...
        ELog(@"No history of connected devices");
    }
}
        ...
        self.manager.stateUpdateBlock = ^(CBCentralManager *central) {
            ...
            switch (central.state) {
                ...
                case CBCentralManagerStatePoweredOff:
                    info = @"Bluetooth is available but not turned on";
                    break;
                case CBCentralManagerStateUnsupported:
                    info = @"SDK does not support";
                    break;
                case CBCentralManagerStateUnauthorized:
                    info = @"App not authorized";
                    break;
                      ...
                      completeBlock:^(HLOptionStage stage, CBPeripheral *peripheral, CBService *service, CBCharacteristic *character, NSError *error) {
                          ...
                          switch (stage) {
                              ...
                              case HLOptionStageSeekServices:{
                                  if (error) {
                                      statusStr = @"Failed to find service";
                                  } else {
                                      statusStr = @"Find service successfully";
                              ...
                              case HLOptionStageSeekCharacteristics:{
                                  if (error) {
                                      statusStr = @"Failed to find properties";
                                  } else {
                                      statusStr = @"Find characteristics successfully";
                                  }
                                  break;
                              }
                              case HLOptionStageSeekdescriptors:{
                                  if (error) {
                                      statusStr = @"Failed to find description of attribute";
                                  } else {
                                      statusStr = @"Find the description of the feature successfully";
                                  }
                                  break;
                              }
                              default:
